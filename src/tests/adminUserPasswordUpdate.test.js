...
describe('adminUserPasswordUpdate', () => {
  ...
  test('Check for invalid sessionId', () => {
    clear();
  ...
  test('Check for when old password is not the correct old password', () => {
    expect(adminUserPasswordUpdate(userId.sessionId, 'wrong_p@ssw0rd', 'new_p@ssword'))
      .toMatchObject({ error: expect.any(String) });
  });
  test('Check for when new password matches the old password exactly', () => {
    expect(adminUserPasswordUpdate(userId.sessionId, 'p@ssw0rd', 'p@ssw0rd'))
      .toMatchObject({ error: expect.any(String) });
  });
  test('Check for when new password has already been used by this user', () => {
    adminUserPasswordUpdate(userId.sessionId, 'p@ssw0rd', 'new_p@ssw0rd');
    expect(adminUserPasswordUpdate(userId.sessionId, 'new_p@ssw0rd', 'p@ssw0rd'))
      .toMatchObject({ error: expect.any(String) });
  });
  test('Check for when new password is less than 8 characters', () => {
    expect(adminUserPasswordUpdate(userId.sessionId, 'p@ssw0rd', 'p@ssw0r'))
      .toMatchObject({ error: expect.any(String) });
  });
  test('Check for when new password does not contain at least one number', () => {
    expect(adminUserPasswordUpdate(userId.sessionId, 'p@ssw0rd', 'p@ssword'))
      .toMatchObject({ error: expect.any(String) });
  });
  test('Check for when new password does not contain at least one letter', () => {
    expect(adminUserPasswordUpdate(userId.sessionId, 'p@ssw0rd', '12345678'))
      .toMatchObject({ error: expect.any(String) });
  });
  test('No errors', () => {
    adminUserPasswordUpdate(userId.sessionId, 'p@ssw0rd', 'valid_p@ssw0rd');
