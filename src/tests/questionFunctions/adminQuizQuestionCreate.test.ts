...
describe('Testing POST /v2/admin/quiz/{quizid}:', () => {
  ...
  test('ThumbnailUrl does not end with jpg, jpeg, png.', () => {
    questionBody.thumbnailUrl = 'http://example.com/birb.webp';
    expect(() => requestQuizQuestionCreateV2(token, quizId, questionBody)).toThrow(HTTPError[400]);
  });
  ...
  test('Failed test: invalid quiz.', () => {
    expect(() => requestQuizQuestionCreateV2(token, quizId + 1, questionBody)).toThrow(HTTPError[403]);
  });
  test('Failed test: user provided does not own quiz.', () => {
    const { jsonBody } = requestAuthRegister('doffy@gmail.com', 'String-Str1ng', 'Donquixote', 'Doflamingo');
    const otherToken = jsonBody.token as string;
    expect(() => requestQuizQuestionCreateV2(otherToken, quizId, questionBody)).toThrow(HTTPError[403]);
  });
});
